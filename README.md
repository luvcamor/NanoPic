...
- 支持 `JPG`、`PNG`、`WEBP`、`GIF`、`BMP`、`TIFF`、`ICO` 格式输入与输出
- 支持按压缩强度或目标体积两种方式压缩
- 支持尺寸调整、背景填充、亮度处理、文字水印与照片方向自动摆正
- 支持覆盖原文件、保留目录结构或输出到指定文件夹
- 支持命名预设、日期时间命名、序号控制和日志记录
...
- 支持按 EXIF 信息自动摆正照片方向
- 支持背景填充与亮度调整
- 支持文字水印：五种固定位置与随机位置，颜色可视化取色，透明度、字号、边距可调
- 支持移除拍摄信息等元数据
- 支持将备注文字写入图片详细信息，资源管理器中可见
...
- 支持保留原文件夹结构
- 支持输出到指定独立文件夹
- 支持命名方式预设与起始序号（详见下方“命名规则”）
- 重复输出策略：覆盖（默认）、自动重命名、跳过或报错

### 效率与界面

- 并发数 1–64 可调，实时生效
- 可选 GPU 加速与窗口置顶
- 常用操作覆盖键盘快捷键，支持纯键盘完成一次压缩

## 命名规则

命名方式为下拉预设：**保持原文件名**（默认）、原文件名 + 序号、仅序号、日期时间，以及自定义模板。日期时间取文件的修改时间。
...
- `{index}`：输出序号
- `{ext}`：目标扩展名，不包含点号
- `{date}`：文件修改日期，如 `2024-05-06`
- `{time}`：文件修改时间，如 `07-08-09`
...
- `{index}` -> `1.jpg`
- `{date}_{time}` -> `2024-05-06_07-08-09.jpg`
